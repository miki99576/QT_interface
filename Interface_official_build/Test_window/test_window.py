	...
	#print(str(question_val))
		if event== "-START-":
			...
			for i in range(len(question_val)):
				window.extend_layout(window['-ROW_PANEL-'],generate_buttons(question_val[i],True)[3])
				print(question_val)
...
# if the button is clicked it becomes unclickable
		for i in range(len(question_val)):
			if event== generate_buttons(question_val[i],True)[1]:
				window[generate_buttons(question_val[i],True)[1]].update(disabled=True,button_color = ('black','yellow'))
				window[generate_buttons(question_val[i],True)[2]].update(disabled=True)
				Score=Score+1
        
			if event== generate_buttons(question_val[i],True)[2]:
				window[generate_buttons(question_val[i],True)[1]].update(disabled=True)
				window[generate_buttons(question_val[i],True)[2]].update(disabled=True,button_color = ('black','yellow'))
